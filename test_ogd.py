...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct  8 13:03:30 2017
...
result = backtester.backTesting(centerdata=True,stepsize=stepsize)
...
# 
nppath = os.getcwd() + '/' + args.timeSeriesCSV.replace('.csv','').replace('data/','') + '/' + str(horizon) + 'step/ogd/np/'  
if not os.path.exists(nppath):
    os.makedirs(nppath)

nppath = nppath + "{0:.4e}".format(stepsize)
np.save(nppath + '_error',resultarray)
np.save(nppath + '_prediction',predictionarray)
np.save(nppath + '_param',paramarray)

confipath = os.getcwd() + '/' + args.timeSeriesCSV.replace('.csv','').replace('data/','') + '/' + str(horizon) +'step/ogd/config/' 
if not os.path.exists(confipath):
    os.makedirs(confipath)   

confipath = confipath + "{0:.4e}".format(stepsize)  + '_testconfig.o'
with open(confipath,'a') as confiFile:
    confiFile.write('input time series CSV:' + args.timeSeriesCSV + '\n')
    confiFile.write('step size for OGD:' + "{0:.4e}".format(stepsize) + '\n')
